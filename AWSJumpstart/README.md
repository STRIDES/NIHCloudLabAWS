...
If you are running bioinformatic workflows, you can leverage the serverless functionality of AWS using the [AWS Genomics CLI](https://aws.amazon.com/genomics-cli/), which is kind of a wrapper for Nextflow and Cromwell with Batch (plus much more). There are some peculiarities to using the CLI within Cloud Lab, so please read this [documentation](/docs/agc) for Cloud Lab specific instructions.
